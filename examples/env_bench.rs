...
fn run(code: &str, gas: u64) -> (Result<String, String>, u64) {
    ...
    for expr in &exprs {
        result = match lisp_eval(expr, &mut env, &mut g) { Ok(v) => v, Err(e) => return (Err(e), gas-g) };
    }
...
fn main() {
    ...
    for n in [100, 1000, 10000] {
        ...
        println!("  count({}): {} internal gas ({:.1} gas/n)", n, used, used as f64 / n as f64);
    }
    ...
    for n in [100, 1000, 10000] {
        ...
        let code = format!("(loop ((i 0)) (if (= i {}) i (recur (+ i 1))))", n);
        let (_, used) = run(&code, 1_000_000_000);
        println!("  count({}): {} gas ({:.1}/n)", n, used, used as f64 / n as f64);
    }
    ...
    for extra in [0, 10, 50, 100] {
        ...
        let (_, used) = run(&code, 1_000_000_000);
        println!("  {} extra bindings, count(1000): {} gas ({:.1}/n)", extra, used, used as f64 / 1000.0);
    }
}