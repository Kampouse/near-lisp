...
async fn deploy_lisp(gas_limit: u64) -> anyhow::Result<(Worker<Sandbox>, near_workspaces::Contract)> {
    let worker: Worker<Sandbox> = near_workspaces::sandbox().await?;
    let wasm = std::fs::read("target/near/near_lisp/near_lisp.wasm")?;
    let contract = worker.dev_deploy(&wasm).await?;
    contract.call("new").args_json(serde_json::json!({ "eval_gas_limit": gas_limit }))
...
async fn test_sandbox_arithmetic() -> anyhow::Result<()> {
    ...
    let r: String = c.call("eval").args_json(serde_json::json!({ "code": "(+ 1 2)" })).max_gas().transact().await?.json()?;
    assert_eq!(r, "3");
    let r: String = c.call("eval").args_json(serde_json::json!({ "code": "(* 6 7)" })).max_gas().transact().await?.json()?;
    assert_eq!(r, "42");
    println!("✓ arithmetic");
    Ok(())
}
...
async fn test_sandbox_lambda_and_closure() -> anyhow::Result<()> {
    let (_, c) = deploy_lisp(50_000).await?;
    let r: String = c.call("eval").args_json(serde_json::json!({ "code": "(define square (lambda (n) (* n n))) (square 9)" })).max_gas().transact().await?.json()?;
    assert_eq!(r, "81");
...
async fn test_sandbox_policy() -> anyhow::Result<()> {
    let (_, c) = deploy_lisp(50_000).await?;
    let pass: bool = c.call("check_policy").args_json(serde_json::json!({
        "policy": "(and (>= score 85) (<= duration 3600))",
        "input_json": "{\"score\": 90, \"duration\": 1200}"
    })).max_gas().transact().await?.json()?;
    assert!(pass);
    let fail: bool = c.call("check_policy").args_json(serde_json::json!({
        "policy": "(and (>= score 85) (<= duration 3600))",
...
async fn test_sandbox_save_eval_policy() -> anyhow::Result<()> {
    let (_, c) = deploy_lisp(50_000).await?;
    c.call("save_policy").args_json(serde_json::json!({
        "name": "quality-gate",
        "policy": "(and (>= score 80) (str-contains status \"complete\"))"
    })).max_gas().transact().await?.into_result()?;

    let r: String = c.call("eval_policy").args_json(serde_json::json!({
        "name": "quality-gate",
...
async fn test_sandbox_near_builtins() -> anyhow::Result<()> {
    let (_, c) = deploy_lisp(50_000).await?;
    let r: String = c.call("eval").args_json(serde_json::json!({ "code": "(near/block-height)" })).max_gas().transact().await?.json()?;
    assert!(r.parse::<i64>().is_ok());
    let r: String = c.call("eval").args_json(serde_json::json!({ "code": "(near/timestamp)" })).max_gas().transact().await?.json()?;
    assert!(r.parse::<i64>().is_ok());
...
async fn test_sandbox_list_ops() -> anyhow::Result<()> {
    let (_, c) = deploy_lisp(50_000).await?;
    let r: String = c.call("eval").args_json(serde_json::json!({ "code": "(car (list 1 2 3))" })).max_gas().transact().await?.json()?;
    assert_eq!(r, "1");
    let r: String = c.call("eval").args_json(serde_json::json!({ "code": "(cdr (list 1 2 3))" })).max_gas().transact().await?.json()?;
    assert_eq!(r, "(2 3)");
    let r: String = c.call("eval").args_json(serde_json::json!({ "code": "(append (list 1 2) (list 3 4))" })).max_gas().transact().await?.json()?;
    assert_eq!(r, "(1 2 3 4)");
    println!("✓ list ops");
    Ok(())
}
...
async fn test_sandbox_storage() -> anyhow::Result<()> {
    let (_, c) = deploy_lisp(50_000).await?;
    let r: String = c.call("eval").args_json(serde_json::json!({ "code": "(near/storage-write \"k\" \"v\")" })).max_gas().transact().await?.json()?;
    assert_eq!(r, "true");
    let r: String = c.call("eval").args_json(serde_json::json!({ "code": "(near/storage-read \"k\")" })).max_gas().transact().await?.json()?;
    assert_eq!(r, "\"v\"");
    let r: String = c.call("eval").args_json(serde_json::json!({ "code": "(near/storage-read \"missing\")" })).max_gas().transact().await?.json()?;
    assert_eq!(r, "nil");
    println!("✓ storage");
    Ok(())
}
