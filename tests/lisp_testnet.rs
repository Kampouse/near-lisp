...
async fn test_lisp_testnet_full_lifecycle() -> anyhow::Result<()> {
    ...
    let root = Account::from_secret_key(account_id.parse()?, secret_key.parse()?, &worker);
    ...
    let pass: bool = contract
        ...
        .json()?;
    assert!(pass);
    println!("✓ policy pass = {}", pass);

    let fail: bool = contract
        .call("check_policy")
        .args_json(serde_json::json!({
            "policy": "(and (>= score 85) (<= duration 3600))",
    ...
    c2.call("new")
        .args_json(serde_json::json!({ "eval_gas_limit": 50 }))
        .max_gas()
        .transact()
        .await?
        .into_result()?;
